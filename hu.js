...
;(function(){
		...
		if (typeof a === "string") {
			...
			if (m) {
				...
				if (/^svg$/i.test(n.tagName)) {
					...
					hu('<rect', n).attr({width:"100%", height:"100%", opacity:0});
				}
				return n;
			}
	...
	var hu = window.hu = window.hu = function(a, c){
	...
	hu.fn = fn; // so that hu can be easily extended
	...
	fn.def = function(a){
		...
		while (p) {
			if (p.n.tagName === "svg") {
				(hu("defs", p)||hu('<defs', p.n)).n.appendChild(u.n); 
				return u.autoid();
			}
			p = hu(p.parentNode);
		}
	...
	for (var n in fn) {
		if (typeof fn[n] === "function") hu[n] = fn[n];
	}
